'''
Author: mengzonefire
Date: 2021-08-05 09:51:26
LastEditTime: 2023-01-08 13:05:53
...
class messageHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        req_datas = self.rfile.read(
            int(self.headers['content-length']))  # 加上限制读取的报文长度
        ...
        if 'sharelink' in jsonData and 'pw' in jsonData:
            respon = bdpanHelper(
                jsonData['sharelink'], jsonData['pw']).getBdlink()  # 单文件分享
            if respon['errno']:
            ...
            self.wfile.write(
                dumps({'errno': 114514, 'msg': '接口参数错误!'}).encode('utf-8'))
