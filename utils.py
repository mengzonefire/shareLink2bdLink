...
LastEditTime: 2023-04-25 21:36:11
...
def readCookie():
    ...
    if not os.path.isfile(cookie_file):
        print(
            f"cookie文件{cookie_file}不存在, 请使用浏览器Cookie Editor插件导出度盘cookie, 在程序所在目录创建{cookie_file}并复制进去"
        )
        return False
    ...
    logid = str(
        base64.b64encode(unquote(baidu_cookies_dict["BAIDUID"]).encode("utf-8")),
        "utf-8",
    )
    return True
...
class bdpanHelper:
        ...
        for fileInfo in fileInfoList:
            ...
            if 0 == result:
                outputInfoList.append(
                    {
                        "path": fileInfo["path"],
                        "bdlink": f"{fileInfo['md5']}#{fileInfo['size']}#{fileInfo['path'][1:]}",
                        "errno": 0,
                    }
                )
            elif 404 == result:
                ...
                outputInfoList.append({"path": fileInfo["path"], "errno": result})
    ...
    def verify(self):
        ...
        if len(_bdstoken):
            ...
            return {"errno": 810}
        respon = s.post(
            verify_url.format(surl=self.surl, bdstoken=self.bdstoken),
            headers={
    ...
    def getMd5FromDlink(self, fileInfo):
        ...
        timestamp: int
        respon = s.get(
            tpl_url.format(surl="1" + self.surl, logid=logid), cookies=self.cookie
        ).text
        respon = json.loads(respon)
        if 0 == respon["errno"]:
        ...
        respon = s.post(
            sharedownload_url.format(sign=sign, timestamp=timestamp),
            cookies=self.cookie,
            data=data,
        ).text
        respon = json.loads(respon)
        if 0 == respon["errno"]:
            ...
            return {"path": fileInfo["path"], "errno": respon["errno"]}
        respon = s.get(dlink, headers={"Range": "bytes=0-1", "User-Agent": ua_dl})
        ...
        return {
            "path": fileInfo["path"],
            "bdlink": f"{fileInfo['md5']}#{fileInfo['size']}#{fileInfo['path']}",
            "errno": 0,
        }
    ...
    def checkMd5(self, fileInfo):
        if not fileInfo["md5"]:
            return {"errno": 404}
        data = {
            "block_list": json.dumps([fileInfo["md5"]]),
            "size": fileInfo["size"],
            "path": testPath,
            "isdir": 0,
            "rtype": 3,
        ...
        respon = s.post(
            create_url + f"?bdstoken={self.bdstoken}",
            cookies=baidu_cookies_dict,
            data=data,
        ).text
        respon = json.loads(respon)
        if 0 == respon["errno"]:
            return {"errno": 0}
        elif 31190 == respon["errno"]:
            return {"errno": 404}
        else:
            return {"errno": respon["errno"]}
...
if __name__ == "__main__":
    readCookie()
    respon = bdpanHelper(
        "https://pan.baidu.com/s/1eShrmBBtKqQCP3zHjIOxkw?pwd=ais4", "ais4"
    ).getBdlink()  # 模块测试
    if 0 == respon["errno"]:
        print(respon)
    else:
